...
.PHONY: dockerimages
dockerimages:
	docker build -t mwennrich/csi-lvm-provisioner:${DOCKER_TAG} . -f cmd/provisioner/Dockerfile
	docker build -t mwennrich/csi-lvm-controller:${DOCKER_TAG} . -f cmd/controller/Dockerfile

.PHONY: dockerpush
dockerpush:
	docker push mwennrich/csi-lvm-controller:${DOCKER_TAG}
	docker push mwennrich/csi-lvm-provisioner:${DOCKER_TAG}
...
tests:
	...
	@sh -c '. ./tests/files/.dockerenv && docker build -t mwennrich/csi-lvm-provisioner:${DOCKER_TAG} . -f cmd/provisioner/Dockerfile'
	@sh -c '. ./tests/files/.dockerenv && docker build -t mwennrich/csi-lvm-controller:${DOCKER_TAG} . -f cmd/controller/Dockerfile'
